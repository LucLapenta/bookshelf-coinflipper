...
Recently I find myself flipping a coin often to make simple decisions. It's
simple, flip a coin to decide between two choices when you are unsure. But, if
you don't like the result of the coin you just pick the other choice. The
omniscient coin has now revealed your true feelings. There is one rule: only
one flip is allowed. You need to either accept or deny the coin, no redos.
...
My issue with relying on this method is that I never have coins lying around anymore. Sure you can use [Google coinflip]() or ask Alexa to flip a count, but I like things that are tactile.

On the other hand, its essentially instant to just generate a random number and assign it to heads or tails, but one of the best parts of flipping a coin is the suspense! There is that moment while the coin is flipping and you are just hanging on by a thread waiting for it to fall.
